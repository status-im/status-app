...
Item {
    ...
    TestCase {
        ...
        // How the count behaves well past the settle point: if it keeps moving,
        // the settle point is early rather than the surface non-deterministic.
        function drainTrajectory(label, settledCount) {
            let line = "TRAJ %1 settle=%2".arg(label).arg(settledCount)
            let visibleTags = function() {
                const tags = probe.findAllByObjectNamePrefix(d.detailItem(),
                                                             "assetsDetailsHeaderChainTag_")
                let v = 0
                for (const t of tags) if (t.visible) v++
                return "%1/%2".arg(v).arg(tags.length)
            }
            const tags = probe.findAllByObjectNamePrefix(d.detailItem(),
                                                         "assetsDetailsHeaderChainTag_")
            for (const t of tags) {
                const attached = probe.findAllByTypePrefix(t, "QQuickLayoutAttached")
                console.info("TAG %1 %2 visible=%3 w=%4 attached=%5"
                             .arg(label).arg(t.objectName).arg(t.visible)
                             .arg(t.width).arg(attached.length))
            }
            line += " tagsVisible=" + visibleTags()
            for (const ms of [100, 200, 500, 1000, 2000]) {
                d.spin(ms)
                line += " +%1ms=%2(%3)".arg(ms).arg(probe.countObjects(d.detailItem()))
                        .arg(visibleTags())
            }
            console.info(line)
        }

        // Resolving a QQuickLayout's implicit size runs `ensureLayoutItemsUpdated`,
        // which is what allocates a child's `Layout` attached object.
        function forceLayouts() {
            let n = 0
            for (const prefix of ["QQuickRowLayout", "QQuickColumnLayout",
                                  "QQuickGridLayout", "QQuickStackLayout"]) {
                for (const l of probe.findAllByTypePrefix(d.detailItem(), prefix)) {
                    void l.implicitWidth
                    void l.implicitHeight
                    n++
                }
            }
            return n
        ...
        function census(label, settled) {
            const all = probe.findAllByTypePrefix(d.detailItem(), "")
            let hist = ({})
            for (const o of all) {
                const t = probe.typeName(o).replace(/_QMLTYPE_[0-9]+$/, "")
                hist[t] = (hist[t] || 0) + 1
            }
            let lines = []
            for (const o of all) {
                if (probe.typeName(o) !== "QQuickLayoutAttached")
                    continue
                const owner = probe.parentObject(o)
                lines.push("LAYOUTATTACHED %1 %2 %3 %4 %5"
                           .arg(label).arg(settled)
                           .arg(owner ? probe.typeName(owner) : "<none>")
                           .arg(owner && owner.objectName ? owner.objectName : "-")
                           .arg(owner ? probe.typeName(probe.parentObject(owner)) : "-"))
            }
            for (const t of Object.keys(hist).sort())
                lines.push("HIST %1 %2 %3 %4".arg(label).arg(settled).arg(hist[t]).arg(t))
            console.info("CENSUS-BEGIN %1 settled=%2 walked=%3\n%4\nCENSUS-END"
                         .arg(label).arg(settled).arg(all.length)
                         .arg(lines.join("\n")))
        }
        ...
        function openDetail(phase) {
            ...
            const forced = forceLayouts()
            const after = probe.countObjects(d.detailItem())
            console.info("FORCE %1 settled=%2 layouts=%3 after=%4"
                         .arg(phase).arg(current).arg(forced).arg(after))
            census(phase, current)
            drainTrajectory(phase, current)
