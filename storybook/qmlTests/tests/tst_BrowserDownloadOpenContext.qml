...
Item {
    ...
    TestCase {
        ...
        function createContext() {
            ...
            ctx.isKnownTargetPathFn = (path) => knownTargets.indexOf(path) >= 0
        ...
        function test_isBrowsableLocalUrl_onlyPlayerPagesAndOwnTargets() {
            ...
            const rec = completed("video/webm", "clip 1.webm")
            const target = String(rec.targetPath)
            knownTargets = [target]

            verify(ctx.isBrowsableLocalUrl(ctx.mediaPlayerPageUrl(rec)))

            // The Record's own Download Target, percent-encoded as WebEngine reports it.
            verify(target.indexOf("clip 1.webm") > 0)
            verify(ctx.isBrowsableLocalUrl(
                       "file://" + target.split("/").map(encodeURIComponent).join("/")))

            // Anything else local stays blocked by the Backend guard.
            verify(!ctx.isBrowsableLocalUrl("file:///etc/passwd"))
            verify(!ctx.isBrowsableLocalUrl("file:///tmp/status-player/evil.html"))
            verify(!ctx.isBrowsableLocalUrl(
                       "file://" + target.replace("clip 1.webm", "other.webm")))
            verify(!ctx.isBrowsableLocalUrl("https://example.com/clip.webm"))
            verify(!ctx.isBrowsableLocalUrl(""))
        }
