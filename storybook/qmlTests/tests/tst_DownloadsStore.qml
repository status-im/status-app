...
Item {
    ...
    TestCase {
        ...
        function test_isKnownTargetPath_matchesRecordTargetsOnly() {
            ...
            verify(store.isKnownTargetPath(target))
            verify(!store.isKnownTargetPath(target.replace("clip 1.webm", "other.webm")))
            verify(!store.isKnownTargetPath("/etc/passwd"))
            verify(!store.isKnownTargetPath(""))
        }
