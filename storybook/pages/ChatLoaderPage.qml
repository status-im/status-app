...
SplitView {
    ...
    // Diagnostic: dump the chat list's delegate count and the sizing chain
    // that controls its virtualization
    Timer {
        id: probeTimer
        interval: 2500
        onTriggered: {
            ...
            const cl = findByName(loader, "ContactsColumnView_chatList")
            if (!cl) { console.info("probe: chat list not found"); return }
            const lv = cl.statusChatListItems
            console.info("probe: delegates", lv.contentItem.children.length, "of", lv.count,
                         "lvH", lv.height, "contentH", lv.contentHeight)
            let p = lv
            let chain = []
            while (p) {
                chain.push((p.objectName || p.toString().split("(")[0]) + ":" + Math.round(p.height))
                p = p.parent
            }
            console.info("probe chain:", chain.join(" < "))
        }
    }
