## Proxy-chain propagation benchmark (RED baseline for the wake-stall series).
##
...
## model — reproduce the ×N amplification the terminal-model commit must remove.
##
...
## whole-model dataChanged; the terminal-model commits (Phase C/D) flip those.
...
when isMainModule:
  ...
  #     rows.
  # RED baseline (labelled: the terminal-model commit flips these): a single-cell
