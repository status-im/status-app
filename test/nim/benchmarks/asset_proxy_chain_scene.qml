...
//
// Why two real LeftJoinModels (the earlier scene pre-joined and dropped them):
// a right-model cell change makes LeftJoinModel emit a WHOLE-model dataChanged
// (leftjoinmodel.cpp:285), the ×N amplification the terminal-model commit must
// remove. tokenGroups feeds the inner join, whose output is the RIGHT model of
