...
@allure.title('Settings — Unlock keycard with recovery phrase')
@allure.description(
    'Password profile → Settings → Read Keycard → import seed onto empty card → '
    'block card with wrong PIN → unblock with recovery phrase and set new PIN → '
    'success → Read Keycard with new PIN shows key pair again'
)
def test_settings_unblock_keycard_with_recovery_phrase(keycard_simulator, main_window, user_account):
    ...
    keycard_mng_popup = keycard_settings.unblock_with_recovery()
    keycard_mng_popup.unblock_with_recovery_phrase(NEW_PIN, seed_phrase.split())
