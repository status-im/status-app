...
def test_check_language_selector(main_window, user_account):
    ...
    with step('Verify user can change language on onboarding screen'):
        ...
        assert welcome_screen.language_selector.text == LanguageCodes.ENGLISH.value, f'English should be default'
        ...
        assert welcome_screen.create_profile_button.text != 'Create profile', f'Language was not changed'

        welcome_screen.open_language_selector().select_language(LanguageCodes.ENGLISH.value.lower())

        assert welcome_screen.language_selector.text == LanguageCodes.ENGLISH.value, f'Language was not changed'
