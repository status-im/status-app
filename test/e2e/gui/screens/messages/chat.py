...
from gui.screens.messages.message import Message, _nudge_outgoing_header
from helpers.chat_helper import plain_text_from_message_object
...
class ChatMessagesView(QObject):
    ...
    def remove_member_from_chat(self, member):
        ...
        from gui.screens.messages.panels import ToolBar
...
class MessageQuickActions(QObject):
    ...
    def _open_context_menu(self):
        ...
        if not driver.waitFor(menu_ready, timeout_msec):
            raise TimeoutError(
                f'Message context menu was not ready within {timeout_msec} ms; '
                f'last error was {last_error!r}'
            )
