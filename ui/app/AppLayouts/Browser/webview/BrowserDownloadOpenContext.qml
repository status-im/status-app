 ...
 * Download Records our Backends can render, how the player page for local
 * media is assembled, and which file:// URLs the Backend's local-browsing
 * guard may let through.
 *
...
QtObject {
    ...
    /// True when a path is the Download Target of one of our Records
    /// (the Record half of the local-URL guard exception).
    property var isKnownTargetPathFn: function(path) { return false }
    ...
    /// The only file:// URLs the browser may navigate to: player pages we
    /// generated and the Download Targets of Records the user downloaded.
    /// Everything else stays blocked by the Backend's local-browsing guard
    /// (WebViewAdapter).
    function isBrowsableLocalUrl(url) {
        ...
        return !!root.isKnownTargetPathFn && root.isKnownTargetPathFn(path)
    }
