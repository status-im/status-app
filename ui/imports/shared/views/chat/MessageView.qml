...
Loader {
    ...
    function openMessageContextMenu(point, selectedText = "", openOverMessage = false, initiallyExpanded = false, positionProvider = null, closeOnHoverExit = false) {
        ...
        const openCollapsedOnMobile = StatusQUtils.Utils.isMobile
        const effectiveInitiallyExpanded = openCollapsedOnMobile ? false : initiallyExpanded

        if (openCollapsedOnMobile && d.contextMenu?.opened && !d.contextMenu.initiallyExpanded)
            return
        ...
        const params = {
            ...
            collapsedSingleRow: !effectiveInitiallyExpanded && d.isLandscape,
            deferCloseOnPressOutside: openCollapsedOnMobile
        }
        ...
        const popupPoint = positionProvider ? positionProvider(d.contextMenu)
                                            : (openCollapsedOnMobile ? d.messageContextMenuPointForMobile(d.contextMenu)
                                                                     : (openOverMessage ? d.messageContextMenuPointOverMessage(point, d.contextMenu) : point))
    ...
    QtObject {
        ...
        readonly property bool opensMessageContextMenuOnLongPress: StatusQUtils.Utils.isMobile
        readonly property bool opensMessageContextMenuOnRightClick: !StatusQUtils.Utils.isMobile
        readonly property bool opensMessageContextMenuOnHover: !StatusQUtils.Utils.isMobile
        ...
        function messageContextMenuPointOverMessage(point, contextMenu) {
            if (!root.item || !contextMenu)
                return point

            const menuWidth = contextMenu.maxImplicitWidth || contextMenu.implicitWidth
            const menuHeight = contextMenu.implicitHeight
            const x = Math.max(0, (root.item.width - menuWidth) / 2)
            const y = Math.max(0, Math.min(point.y - menuHeight / 2, root.item.height - menuHeight))

            return Qt.point(x, y)
        }

        function messageContextMenuPointForMobile(contextMenu) {
            if (!root.item || !contextMenu)
                return Qt.point(0, 0)

            const menuWidth = contextMenu.maxImplicitWidth || contextMenu.implicitWidth
            const x = Math.max(0, root.item.width - menuWidth - Theme.padding)
            return Qt.point(x, 0)
        }

        function messageContextMenuPointForHover(anchorItem, contextMenu) {
            if (!root.item || !anchorItem)
                return Qt.point(0, 0)

            const menuWidth = contextMenu?.maxImplicitWidth || 234
            const menuHeight = contextMenu?.implicitHeight || 0
            const x = Math.max(0, anchorItem.width - menuWidth - Theme.padding)
            const y = -menuHeight / 2
            return anchorItem.mapToItem(root.item, x, y)
        }

        function openHoverMessageContextMenu(anchorItem, selectedText) {
            if (!d.opensMessageContextMenuOnHover || !anchorItem || anchorItem.hideQuickActions || root.isViewMemberMessagesePopup)
                return

            if (d.contextMenu?.opened && d.contextMenuOpenedFromHover && d.contextMenuHoverAnchor === anchorItem)
                return

            d.contextMenu?.close()
            d.contextMenuHoverAnchor = anchorItem
            openMessageContextMenu(Qt.point(0, 0), selectedText, false, false,
                                   contextMenu => d.messageContextMenuPointForHover(anchorItem, contextMenu),
                                   true)
        }

        function scheduleHoverMessageContextMenuClose() {
    ...
    Timer {
        ...
        interval: 350
        onTriggered: {
            if (d.contextMenuClosesOnHoverExit && d.contextMenu?.opened && !d.contextMenu.initiallyExpanded && !d.contextMenu.expanded && !d.messageHovered && !d.contextMenuHovered)
                d.contextMenu.close()
        }
    }
    ...
    Component {
        ...
        ColumnLayout {
            ...
            StatusMessage {
                ...
                onHoverChanged: (messageId, hovered) => {
                    d.messageHovered = hovered
                    if (hovered) {
                        d.openHoverMessageContextMenu(delegate, delegate.selectedText)
                        return
                    }
                    d.scheduleHoverMessageContextMenuClose()
                }

                onContextMenuRequested: pos => root.openMessageContextMenu(pos, delegate.selectedText, d.opensMessageContextMenuOnLongPress, d.opensMessageContextMenuOnRightClick) // for StatusTextMessage which would eat the press events internally
                TapHandler {
                    enabled: d.opensMessageContextMenuOnLongPress
                    gesturePolicy: TapHandler.ReleaseWithinBounds // exclusive grab on press
                    acceptedDevices: PointerDevice.TouchScreen
                    onLongPressed: root.openMessageContextMenu(point.position, delegate.selectedText, true)
                }

                TapHandler {
                    enabled: d.opensMessageContextMenuOnRightClick
                    acceptedDevices: PointerDevice.Mouse | PointerDevice.TouchPad | PointerDevice.Stylus
                    acceptedButtons: Qt.RightButton
                    onTapped: (point, button) => root.openMessageContextMenu(point.position, delegate.selectedText, false, true)
                }
