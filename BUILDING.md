...
```bash
brew install cmake pkg-config go@1.24 qt protobuf nim
```
...
Make sure you have `~/go/bin` in your `PATH` (status-go's `go generate` puts its
protobuf plugin there; the build installs it for you):
```bash
