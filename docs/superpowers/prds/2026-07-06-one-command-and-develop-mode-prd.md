...
| 0009 develop-mode core | overlay + `develop`/`undevelop`/`vendors` tasks for statusgo + sds, divergence guard, escape hatch; vendor table shaped for 0011/0012; + isaac warm-setup wall fix, parent-worktree rpath alias | done (2026-07-07) |
...
| 0014 prl-to-pc graph adoption | version-tag pin #v0.2.0 (first tag-shaped pin; annotated tag resolves like a #sha special + carries the semantic version), manifest source-only AND srcDir-free (either strips the store copy — new wall sharpening), submodule removed (backup functional), mk/.pc consumed overlay-aware from the store, tools built into repo-local .prl-to-pc-build (store byte-identical across builds), develop round-trip + iOS leg proven; pre-push bring-up = seeded pkgcache (overlay-only cannot survive setup); PUSH PENDING for remote resolvability | done (2026-07-08) |
