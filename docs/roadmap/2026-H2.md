# 2026 H2

## Table of Contents
- [2026 H2](#2026-h2)
  - [Table of Contents](#table-of-contents)
  - [2.39](#239)
    - [Features](#features)
  - [2.40](#240)
    - [Features](#features-1)
  - [2.41](#241)
    - [Features](#features-2)


## 2.39

Release Epic: https://github.com/status-im/status-app/issues/19529

Estimated release: Mid-September

### Features

- ⭐ [Keycard library/service alignment](https://github.com/status-im/status-app/issues/20289)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- ⭐ [SeaQT migration](https://github.com/status-im/status-app/issues/17801)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- ⭐ [Battery consumption improvements](https://github.com/status-im/status-go/issues/7563)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- ⭐ [UI Zoom setting](https://github.com/status-im/status-app/issues/20169)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- ⭐ [Standard mode in Mobile Browser (instead of incognito only)](https://github.com/status-im/status-app/issues/21416)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- ⭐ [Chat Input Field UX Improvements](https://github.com/status-im/status-app/issues/19524)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- ⭐ [SDS phase 3](https://github.com/status-im/status-go/issues/7363)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
  - Fetch missing messages when flagged as missed
  - Enable wrapping of community messages
  - Breaking change for people in 2.36 or lower (first supported release is 2.37)
- [Performance improvements](https://github.com/status-im/status-app/issues/20573)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- [Implement Back navigation](https://github.com/status-im/status-app/issues/20997)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- [Adding Logos Storage protocol as a Community Service provider](https://github.com/logos-storage/logos-storage-nim/issues/1304)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- [Threads POC](https://github.com/status-im/status-app/issues/21257)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- [New translations](https://github.com/status-im/status-app/issues/19512)
  - In Progress ⏳ 🟩🟩🟩🟨⬜ 72%
- Swap + Bridge in one (LI.FI support) (get rid of the old Bridge)
  - [LI.FI integration](https://github.com/status-im/status-app/issues/21246)
    - In Progress ⏳ 🟩🟩🟨⬜⬜ 50%
    - phase 1 (integrate LI.FI into the existing SWAP feature) - Done ✅ 🟩🟩🟩🟩🟩 100%
    - phase 2 (integrate LI.FI bridge feature into the existing SWAP UI) - Done ✅ 🟩🟩🟩🟩🟩 100%
    - phase 3 (improve UX of the SWAP + Bridge feature) - In Progress 🟩🟩🟩🟨⬜ 75%
- [Add bots](https://github.com/status-im/status-app/issues/21861):
  - In Progress ⏳ 🟩🟩🟨⬜⬜ 50%
  - Welcome bot
  - Support bot
- [Improve `New Chat Creation / Group Chats Management` Flow](https://github.com/status-im/status-app/issues/19222)
- [Browser: Add Downloads feature on Mobile](https://github.com/status-im/status-app/issues/21376)
  - Done ✅ 🟩🟩🟩🟩🟩 100%
- [Add a loading screen to show the user when Status fetches a community](https://github.com/status-im/status-app/issues/21089)
  - Done ✅ 🟩🟩🟩🟩🟩 100%

## 2.40

Estimated release: End of October

### Features

- ⭐ [[Logos] help with Logos Basecamp on mobile](https://github.com/status-im/status-app/issues/22331)
  - In Progress ⏳ 🟩⬜⬜⬜⬜ 20%
    
- ⭐ [[Logos core] create a plan on developing Logos wallet components and brining them to Status app (via Logos core or directly)](https://github.com/status-im/status-app/issues/22436)
  - In Progress ⏳ ⬜⬜⬜⬜⬜ 0%
    
- ⭐ [Switch from `nimbus-build-system` to `Nimble`](https://github.com/status-im/status-app/issues/19907)
  - In Progress ⏳ 🟩🟨⬜⬜⬜ 25%

- ⭐ [[Epic] Improve app performance, reduce RAM usage, and data consumption](https://github.com/status-im/status-app/issues/22045)
  - In Progress ⏳ 🟩🟩⬜⬜⬜ 48%

- ⭐ [Threads support](https://github.com/status-im/status-app/issues/21090)
  - In Progress ⏳ ⬜⬜⬜⬜⬜ 5%

- ⭐ [Keycard Shell integration](https://github.com/status-im/status-app/issues/20293)
  - In Progress ⏳ 🟩🟩🟨⬜⬜ 50%
    
- ⭐ [Share feature on Mobile](https://github.com/status-im/status-app/issues/20439)
  - In Progress ⏳ 🟩🟨⬜⬜⬜ 33%
 
- ⭐ [[App] declare to Android and iOS which file formats we support and launch them in Browser](https://github.com/status-im/status-app/issues/21889)
  - To Do ⏳ ⬜⬜⬜⬜⬜ 0%
    

- [Zero onboarding](https://github.com/status-im/status-app/issues/20438)
- Remember password
- [Link prettifying](https://github.com/status-im/status-app/issues/22031)
- [Self-chat for taking notes](https://github.com/status-im/status-app/issues/22033)
- [Spellcheck in the Chat input](https://github.com/status-im/status-app/issues/20333)



## 2.41

Estimated release: Mid-November

### Features

- [Nimbus Verification Proxy](https://github.com/status-im/status-app/issues/19538)
  - By using [eth-rpc-proxy](https://github.com/erpc/erpc)
  - In Progress ⏳
- [Color System Revamp](https://github.com/status-im/status-app/issues/19455)
- [Private Transactions POC](https://github.com/status-im/status-app/issues/19539)
- [Refactor popups](https://github.com/status-im/status-app/issues/20128)
  - In Progress ⏳ 🟩⬜⬜⬜⬜ 25%
- Integrate Puzzle auth on Market Proxy
- [New Global Search (Replace Home + CTRL + K)](https://github.com/status-im/status-app/issues/19220)
- Refactor WalletConnect to use the same connector service as the Browser extension
  - This will use the same storage and reduce duplicated code.
- Revamp main wallet view
- [Improve Send modal UX](https://github.com/status-im/status-app/issues/21284)
- Improve UX of sharing your profile or community
- Record and send audio messages
- Use Fastlane to fetch messages on iOS notifications
- Support custom RPC providers
- [Simplify Network filter behavior](https://github.com/status-im/status-app/issues/18663)
- [Enable making Status the default browser on Mobile](https://github.com/status-im/status-app/issues/20448) 
- [Support custom tokens](https://github.com/status-im/status-app/issues/19691)
- File sending over Codex
  - Dependent on Codex being available in Light mode for mobile and having a C library
- Online voice and video calls
- [UI modularization](https://github.com/status-im/status-app/issues/17872)
  - [FURPS](/docs/FURPS/ui-modularization.md)
  - In Progress ⏳ 🟩⬜⬜⬜⬜ 27%
- [Ethereum Follow Protocol](https://github.com/status-im/status-app/issues/18685)
  - In Progress ⏳ 🟨⬜⬜⬜⬜ 10%
