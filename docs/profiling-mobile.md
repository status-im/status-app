...
| Nim              | `-d:nimTypeNames` is added so Android Studio's CPU profiler can resolve native frames into typed Nim names (e.g. `messages.controller.Controller.init`). Note: Nim is still compiled with `-d:debug`; only the Java/Kotlin/Gradle side is release-like. |
