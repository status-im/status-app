import tables

const
  IPFS_SCHEMA* = "ipfs://"
  IPFS_GATEWAY* = "https://gateway.pinata.cloud/ipfs/"

  ZERO_ADDRESS* = "0x0000000000000000000000000000000000000000"

  ETH_SYMBOL* = "ETH"
  ETH_DECIMALS* = 18
  BNB_SYMBOL* = "BNB"
  BNB_DECIMALS* = 18
  STATUS_SYMBOL* = "SNT"
  STATUS_DECIMALS* = 18
  STATUS_SYMBOL_TESTNET* = "STT"
  STATUS_DECIMALS_TESTNET* = 18

  ETH_GROUP_KEY* = "eth-native"
  BNB_GROUP_KEY* = "bsc-native"
  STATUS_GROUP_KEY* = "status"
  STATUS_TEST_TOKEN_GROUP_KEY* = "status-test-token"

  ETHEREUM_MAINNET* = 1
  ETHEREUM_SEPOLIA* = 11155111
  OPTIMISM_MAINNET* = 10
  OPTIMISM_SEPOLIA* = 11155420
  ARBITRUM_MAINNET* = 42161
  ARBITRUM_SEPOLIA* = 421614
  BSC_MAINNET* = 56
  BSC_TESTNET* = 97
  BASE_MAINNET* = 8453
  BASE_SEPOLIA* = 84532
  LINEA_MAINNET* = 59144
  LINEA_SEPOLIA* = 59141
  UNICHAIN_MAINNET* = 130
  UNICHAIN_SEPOLIA* = 1301
  KATANA_MAINNET* = 747474
  KATANA_BOKUTO* = 737373
  INK_MAINNET* = 57073
  INK_SEPOLIA* = 763373
  ABSTRACT_MAINNET* = 2741
  ABSTRACT_SEPOLIA* = 11124
  ZKSYNC_MAINNET* = 324
  ZKSYNC_SEPOLIA* = 300
  SONEIUM_MAINNET* = 1868
  SONEIUM_MINATO* = 1946
  SCROLL_MAINNET* = 534352
  SCROLL_SEPOLIA* = 534351
  BLAST_MAINNET* = 81457
  BLAST_SEPOLIA* = 168587773
  ROBINHOOD_MAINNET* = 4663
  ROBINHOOD_TESTNET* = 46630
  ETHEREUM_HOODI* = 560048

  SUPPORTED_TEST_NETWORKS*: Table[int, bool] = {
    ETHEREUM_HOODI: true,
    ETHEREUM_SEPOLIA: true,
    OPTIMISM_SEPOLIA: true,
    ARBITRUM_SEPOLIA: true,
    BASE_SEPOLIA: true,
    BSC_TESTNET: true,
    LINEA_SEPOLIA: true,
    UNICHAIN_SEPOLIA: true,
    KATANA_BOKUTO: true,
    INK_SEPOLIA: true,
    ABSTRACT_SEPOLIA: true,
    ZKSYNC_SEPOLIA: true,
    SONEIUM_MINATO: true,
    SCROLL_SEPOLIA: true,
    BLAST_SEPOLIA: true,
    ROBINHOOD_TESTNET: true,
  }.toTable

  ETH_TRANSACTION_TYPE* = "eth"
  ERC20_TRANSACTION_TYPE* = "erc20"
  ERC721_TRANSACTION_TYPE* = "erc721"

  SNT_CONTRACT_ADDRESS* = "0x744d70fdbe2ba4cf95131626614a1763df805b9e"
  STT_CONTRACT_ADDRESS_SEPOLIA* = "0xE452027cdEF746c7Cd3DB31CB700428b16cD8E51"

  SIGNATURE_LEN* = 130
  SIGNATURE_LEN_0X_INCLUDED* = SIGNATURE_LEN + 2
  SIGNATURE_R_LEN* = 64
  SIGNATURE_S_LEN* = 64
  SIGNATURE_V_LEN* = 2

  TX_HASH_LEN* = 32 * 2
  TX_HASH_LEN_WITH_PREFIX* = TX_HASH_LEN + 2

  PROCESSOR_NAME_TRANSFER* = "Transfer"
  PROCESSOR_NAME_BRIDGE_HOP* = "Hop"
  PROCESSOR_NAME_BRIDGE_CELER* = "CBridge"
  PROCESSOR_NAME_SWAP_PARASWAP* = "Paraswap"
  PROCESSOR_NAME_SWAP_LIFI* = "LiFi"
  PROCESSOR_NAME_SWAP_RELAY* = "Relay"
  PROCESSOR_NAME_ERC721* = "ERC721Transfer"
  PROCESSOR_NAME_ERC1155* = "ERC1155Transfer"
  PROCESSOR_NAME_ENS_REGISTER* = "ENSRegister"
  PROCESSOR_NAME_ENS_RELEASE* = "ENSRelease"
  PROCESSOR_NAME_ENS_PUBLIC_KEY* = "ENSPublicKey"
  PROCESSOR_NAME_STICKERS_BUY* = "StickersBuy"

  STATUS_TOKEN_ADDRESSES*: Table[int, string] = {
    ETHEREUM_MAINNET: "0x744d70fdbe2ba4cf95131626614a1763df805b9e",
    OPTIMISM_MAINNET: "0x650af3c15af43dcb218406d30784416d64cfb6b2",
    ARBITRUM_MAINNET: "0x707f635951193ddafbb40971a0fcaab8a6415160",
    BASE_MAINNET: "0x662015ec830df08c0fc45896fab726542e8ac09e",
    ETHEREUM_SEPOLIA: "0xE452027cdEF746c7Cd3DB31CB700428b16cD8E51",
    ETHEREUM_HOODI: "0x0B5DAd18B8791ddb24252B433ec4f21f9e6e5Ed0",
    BASE_SEPOLIA: "0xfdb3b57944943a7724fcc0520ee2b10659969a06",
  }.toTable

  TOKEN_KEY_DELIMITER* = "-"
  COMMUNITY_TOKEN_KEY_DELIMITER* = TOKEN_KEY_DELIMITER
