...
##
## The by-key token lookup used to treat "backend says not found" as a non-event:
## nothing was cached, so every repeat lookup of a missing key re-fired a blocking
## GUI-thread RPC. These helpers make negative results first-class and keep
## the refresh-apply path swap-not-clear, so they can be unit-tested without the
## backend or a live Service.
