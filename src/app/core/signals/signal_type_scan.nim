...
## `{"type":"<t>","event":{...}}`. The manager used to `parseJson` the whole
## payload before discovering that nobody handles the type, so a large payload
## of an unhandled type (e.g. a ~222KB local-notifications event) was fully
## decoded on the Qt main thread and then thrown away.
##
## This module inverts that: extract the envelope type with a substring scan
## (no JSON parse) and check it against the known `SignalType` set. Only known
## types are worth a full decode; unknown/unhandled ones are counted and
## dropped untouched.
