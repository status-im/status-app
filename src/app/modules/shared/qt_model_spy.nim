...
template recordCall(c: SignalCall) =
  if globalSpy != nil and globalSpy.enabled:
    globalSpy.calls.add(c)

proc recordBeginInsertRows*(first, last: int) =
  recordCall SignalCall(kind: BeginInsertRows, first: first, last: last)

proc recordEndInsertRows*() =
  recordCall SignalCall(kind: EndInsertRows)

proc recordBeginRemoveRows*(first, last: int) =
  recordCall SignalCall(kind: BeginRemoveRows, first: first, last: last)

proc recordEndRemoveRows*() =
  recordCall SignalCall(kind: EndRemoveRows)

proc recordDataChanged*(topLeft, bottomRight: int, roles: seq[int]) =
  recordCall SignalCall(
    kind: DataChanged,
    topLeft: topLeft,
...
proc recordBeginResetModel*() =
  recordCall SignalCall(kind: BeginResetModel)

proc recordEndResetModel*() =
  recordCall SignalCall(kind: EndResetModel)
...
proc recordEndMoveRows*() =
  recordCall SignalCall(kind: EndMoveRows)
