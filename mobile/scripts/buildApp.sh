...
if [[ "${OS}" == "android" ]]; then
  ...
  cp "$CWD/../android/qt${QT_MAJOR}"/{AndroidManifest.xml,build.gradle,settings.gradle,gradle.properties,google-services.json} "$BUILD_DIR/android-build/"
  ...
  # Simplest fix for local Qt paths: provide QT_ANDROID_DIR env var.
  # build.gradle uses: QT_ANDROID_DIR (env) ?: qtAndroidDir (from gradle.properties)
  QT_PREFIX="$("$QMAKE_BIN" -query QT_INSTALL_PREFIX)"
  export QT_ANDROID_DIR="${QT_PREFIX%/}/src/android/java"
  ...
  # Build with specified gradle targets (prefer wrapper generated by androiddeployqt)
  if [[ -x "./gradlew" ]]; then
