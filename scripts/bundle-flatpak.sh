...
# Stage libsds.so at a known relative path so the manifest can reference it
# as a regular `type: file` source.
mkdir -p tmp/linux/flatpak/in
cp -P vendor/status-go/build/libsds.so tmp/linux/flatpak/in/
