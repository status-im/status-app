...
- The effective backend is the status-go package (the store copy `.nimble-resolution.mk` names as `STATUSGO_SRC`, or a checkout via `STATUSGO_SRC=`). When investigating behavior, trace through the QML or Nim layer into status-go instead of assuming the logic ends in this repository's UI or middleware.
- If the root cause or correct implementation belongs in status-go, modify it there (in a checkout, built with `STATUSGO_SRC=`) instead of adding UI-side or Nim-side workarounds.
...
- Prefer fixes at the owning layer:
  ...
  - status-go (the package: the store copy `.nimble-resolution.mk` names as `STATUSGO_SRC`, or a checkout via `STATUSGO_SRC=`) for backend logic, persistence, protocol, and notification behavior.
...
## Working Style
- When a change spans QML, Nim, and status-go, inspect the full path before editing and keep the final fix in the layer that actually owns the behavior.
